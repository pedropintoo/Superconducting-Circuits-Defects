...
best = "chip_defect_detection/new_dataset_sliced_256_balanced_upsampled_bg50_augmentation_without_mosaic_imgsz_768_2/weights/best.pt"
...
for image_path, result in results:
    ...
    if result_image is None:
        print(f"Warning: could not read result image for {image_path}, skipping display.")
    else:
        result_image = cv2.resize(result_image, (1200, 800))  # Resize for better visibility
        win_name = f"Prediction for {os.path.basename(image_path)}"
        # Start the window thread to avoid Qt moveToThread errors on some builds
        try:
        ...
        # Wait loop: allow closing via key (ESC/q) or the window manager (GUI close)
        while True:
            key = cv2.waitKey(100) & 0xFF
            if key == 27 or key == ord("q"):  # ESC or 'q'
                break
            # If window was closed by the window manager, getWindowProperty returns < 1
            try:
                if cv2.getWindowProperty(win_name, cv2.WND_PROP_VISIBLE) < 1:
                    break
            except Exception:
                # If getWindowProperty fails for this backend, break to avoid infinite loop
                break

        cv2.destroyWindow(win_name)
        # give GUI time to process destroy and clean up
        cv2.waitKey(1)
...
# EVALUATION
# best = "chip_defect_detection/new_dataset_sliced_128_balanced_upsampled_bg50_augmentation_without_mosaic_imgsz_768_/weights/best.pt"
# model = YOLO(best)
